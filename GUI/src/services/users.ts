...
export async function createUser(userData: UserDTO) {
  ...
  const { data } = await apiDev.post<User>('accounts/users/add', {
...
export async function checkIfUserExists(userData: UserDTO) {
  const { data } = await apiDev.post('accounts/users/exists', {
    userIdCode: userData.idCode,
  });
  return data;
}
...
export async function editUser(id: string | number, userData: UserDTO) {
  ...
  const fullName = userData.fullName?.trim();
  const { data } = await apiDev.post<User>('accounts/users/edit', {
    "firstName": fullName?.split(' ').slice(0, 1).join(' ') ?? '',
    "lastName": fullName?.split(' ').slice(1, 2).join(' ') ?? '',
    "userIdCode": id,
    "displayName": userData.displayName,
...
export async function deleteUser(id: string | number) {
  const { data } = await apiDev.post<User>('accounts/users/delete', {
    "userIdCode": id,
  });
  return data;
}
