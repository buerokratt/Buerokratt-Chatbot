...
import { Button, DataTable, Dialog, Drawer, FormMultiselect, HistoricalChat, Icon, Tooltip } from 'components';
...
const ChatOverview: FC<PropsWithChildren<ChatOverViewProps>> = ({ fromDate, untilDate }) => {
  const { t, i18n } = useTranslation();
  const toast = useToast();
  const userInfo = useStore((state) => state.userInfo);
  const routerLocation = useLocation();
  let passedChatId = new URLSearchParams(routerLocation.search).get('chat');
  const passedStartDate = format(new Date(fromDate), 'yyyy-MM-dd');
  const passedEndDate = format(new Date(untilDate), 'yyyy-MM-dd');
  ...
  useEffect(() => {
    const start = format(new Date(fromDate), 'yyyy-MM-dd');
  ...
  useEffect(() => {
    const end = format(new Date(untilDate), 'yyyy-MM-dd');
    setSearchParams((params) => {
      params.set('end', end);
      return params;
    });
  ...
  const getAllEndedChats = useMutation({
    mutationFn: (data: {
      ...
      return apiDev.post('agents/chats/ended', {
        startDate: format(new Date(passedStartDate), 'yyyy-MM-dd'),
        endDate: format(new Date(passedEndDate), 'yyyy-MM-dd'),
  ...
  const chatCommentChangeMutation = useMutation({
    mutationFn: (data: { chatId: string | number; comment: string }) => apiDev.post('comments/history', data),
  ...
  const feedbackTextView = (props: any) => {
    ...
    return (
      <Tooltip content={value}>
        <span style={{ minWidth: '250px' }}>{value.length < 30 ? value : `${value?.slice?.(0, 30)}...`}</span>
      </Tooltip>
    );
  };
  ...
  const statusView = (props: any) => {
    const isLastMessageEvent =
      props.row.original.lastMessageEvent != null && props.row.original.lastMessageEvent !== 'message-read'
  ...
  const filterChatsList = (chatsList: ChatType[]) => {
    const startDate = Date.parse(format(new Date(passedStartDate), 'MM/dd/yyyy'));

    const endDate = Date.parse(format(new Date(passedEndDate), 'MM/dd/yyyy'));
    ...
    <div className={'full-width'}>
      ...
      {statusChangeModal && (
        <Dialog
          ...
          footer={
            <>
              <Button appearance="secondary" onClick={() => setStatusChangeModal(null)}>
                {t('global.cancel')}
              </Button>
              <Button appearance="error" onClick={() => handleChatStatusChange(statusChangeModal)}>
                {t('global.yes')}
              </Button>
            </>
          }
...
export default withAuthorization(ChatOverview, [ROLES.ROLE_ADMINISTRATOR, ROLES.ROLE_CUSTOMER_SUPPORT_AGENT]);
