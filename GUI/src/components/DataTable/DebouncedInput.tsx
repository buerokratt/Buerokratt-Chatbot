...
const DebouncedInput: FC<DebouncedInputProps> = ({ value: initialValue, onChange, debounce = 500, ...props }) => {
  ...
  return (
    <div className="input-container">
      <input style={{ paddingRight: '30px' }} {...props} value={value} onChange={(e) => setValue(e.target.value)} />
      {value && (
        <button style={{ position: 'absolute', right: '0px', bottom: '15px' }} onClick={() => setValue('')}>
          <CloseIcon />
        </button>
      )}
    </div>
