...
VALUES (
    ...
    CASE
        WHEN :customerSupportDisplayName = 'null' THEN NULL ELSE
            :customerSupportDisplayName
    END,
    CASE
        WHEN :customerSupportFirstName = 'null' THEN NULL ELSE :customerSupportFirstName
    END,
    CASE
        WHEN :customerSupportLastName = 'null' THEN NULL ELSE :customerSupportLastName
    END,
    ...
    CASE
        WHEN :feedbackRating = 'null' THEN NULL ELSE
            NULLIF(:feedbackRating::TEXT, '')::INTEGER
    END,
    CASE WHEN :csaTitle = 'null' THEN NULL ELSE :csaTitle END,
    CASE
        WHEN :firstMessageTimestamp = 'null' THEN NULL ELSE
            :firstMessageTimestamp::TIMESTAMP WITH TIME ZONE
    END,
    CASE
        WHEN :lastMessageTimestamp = 'null' THEN NULL ELSE
            :lastMessageTimestamp::TIMESTAMP WITH TIME ZONE
    END,
    CASE WHEN :comment = 'null' THEN NULL ELSE :comment END,
    CASE
        WHEN :commentAddedDate = 'null' THEN NULL ELSE
            :commentAddedDate::TIMESTAMP WITH TIME ZONE
    END,
    CASE WHEN :commentAuthor = 'null' THEN NULL ELSE :commentAuthor END,
    CASE
        WHEN :firstMessage IN ('null', '', 'message-read') THEN NULL ELSE :firstMessage
    END,
    CASE
        WHEN :lastMessage IN ('null', '', 'message-read') THEN NULL ELSE :lastMessage
    END,
    CASE
        WHEN :lastMessageIncludingEmptyContent = 'null' THEN NULL ELSE
            :lastMessageIncludingEmptyContent
    END,
    CASE WHEN :contactsMessage = 'null' THEN NULL ELSE :contactsMessage END,
    CASE
        WHEN :lastMessageEvent = 'null' THEN NULL ELSE
            LOWER(:lastMessageEvent)::EVENT_TYPE
    END,
    CASE
        WHEN :lastMessageEventWithContent = 'null' THEN NULL ELSE
            LOWER(:lastMessageEventWithContent)::EVENT_TYPE
    END,
    CASE
        WHEN (:chatDurationInSeconds::TEXT) = 'null' THEN NULL ELSE
            NULLIF(:chatDurationInSeconds::TEXT, '')::NUMERIC
    END,
    ...
    CASE
        WHEN
            ...
            AND :lastMessage <> ''
            AND :lastMessage <> 'message-read'
            AND :lastMessageEvent <> 'rating'
            AND :lastMessageEvent <> 'requested-chat-forward'
            THEN :lastMessage
    END,
    (CASE
        WHEN
            :lastMessageEvent <> 'rating'
            AND :lastMessageEvent <> 'requested-chat-forward'
            THEN :updated::TIMESTAMP WITH TIME ZONE
    END)::TIMESTAMP WITH TIME ZONE,
    CASE
        WHEN :lastNonEmptyMessageEvent = 'null' THEN NULL ELSE
            LOWER(:lastNonEmptyMessageEvent)::EVENT_TYPE
    END,
