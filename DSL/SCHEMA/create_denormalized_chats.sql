...
WITH combined_records AS (
    ...
    LEFT JOIN LATERAL (
        ...
        FROM content_message_bounds cmb
        CROSS JOIN all_message_bounds amb
        CROSS JOIN customer_messages_count cmc
        LEFT JOIN "message" min_msg ON min_msg.id = cmb.min_content_id
        LEFT JOIN "message" max_content_msg ON max_content_msg.id = cmb.max_content_id
        LEFT JOIN "message" max_msg ON max_msg.id = amb.max_id
    ) m ON TRUE
    LEFT JOIN LATERAL (
        SELECT
    ...
    AND NOT EXISTS (
        SELECT 1 
        FROM chat_history_comments chc_inner 
    ...
    LEFT JOIN "message" m_msg ON m_msg.id = m_bounds.min_id
    LEFT JOIN LATERAL (
        SELECT COUNT(inner_m.id) AS customer_messages_count
        FROM message inner_m
        WHERE 
...
config_records AS (
    ...
    FROM active_chats_at_config_changes acc
    LEFT JOIN LATERAL (
        SELECT * 
        FROM "chat" c
    ...
    LEFT JOIN LATERAL (
        ...
        ORDER BY id DESC
        LIMIT 1
    ) bot_config ON acc.config_key <> 'bot_institution_id'
    LEFT JOIN LATERAL (
        SELECT value
        FROM configuration
        WHERE 
        ...
        ORDER BY id DESC
        LIMIT 1
    ) csa_config ON acc.config_key <> 'is_csa_title_visible'
    WHERE NOT EXISTS (
        SELECT 1 
        FROM chat_history_comments chc 
        WHERE chc.chat_id = acc.chat_id
